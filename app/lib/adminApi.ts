...
// ─── Observability: logs, errors, payment + generation timelines ──────────────
...
export type LogFilters = {
  request_id?: string; user_id?: string; provider_task_id?: string; fingerprint?: string; turn_id?: string;
  task_id?: string; conversation_id?: string; workflow_run_id?: string; job?: string; scope?: string; level?: string;
  search?: string; since?: string; until?: string; limit?: number; direction?: "backward" | "forward";
};
export async function queryLogs(filters: LogFilters) {
  return apiFetch(`/logs${qs(filters)}`);
}

export async function listAppErrors(params: { page?: number; limit?: number; scope?: string; open?: string; since?: string; search?: string } = {}) {
  return apiFetch(`/errors${qs(params)}`);
}
export async function getAppError(id: string) {
  return apiFetch(`/errors/${id}`);
}
export async function resolveAppError(id: string) {
  return apiFetch(`/errors/${id}/resolve`, { method: "POST" });
}

export async function listPaymentEvents(params: { page?: number; limit?: number; user_id?: string; transaction_id?: string; external_user_id?: string; email?: string; provider?: string; store?: string; outcome?: string; event_type?: string; since?: string } = {}) {
  return apiFetch(`/payment-events${qs(params)}`);
}
export async function getPaymentEvent(id: string) {
  return apiFetch(`/payment-events/${id}`);
}

export async function listGenerationEvents(params: { page?: number; limit?: number; provider_task_id?: string; user_id?: string; turn_id?: string; task_id?: string; kind?: string; provider?: string; since?: string } = {}) {
  return apiFetch(`/generation-events${qs(params)}`);
}
export async function getGenerationTrace(providerTaskId: string) {
  return apiFetch(`/generation-events/trace/${encodeURIComponent(providerTaskId)}`);
}
